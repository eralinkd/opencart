if (!customElements.get('button-secondary')) {
  class ButtonSecondary extends HTMLElement {
    constructor() {
      super();

      const template = document.getElementById('button-secondary-template');
      if (!template) {
        console.error('Template "button-secondary-template" not found');
        return;
      }
      ...
      this.button = shadowRoot.querySelector('.button-secondary');
    }
    ...
    updateButton() {
      ...
      this.button.className = `button-secondary button-secondary-${type}`;
